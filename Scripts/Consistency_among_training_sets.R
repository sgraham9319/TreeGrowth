...
growth <- growth_pred(sing_sp, top_mod[, "X0_opt"],
                              top_mod[, "Xb_opt"], top_mod[, "gmax_opt"],
                              top_mod[, "pet_a_opt"], top_mod[, "pet_b_opt"])
combined <- focals %>%
  left_join(x, by = c("tree_id" = "ids")) %>%
...
mod <- best_strs %>%
  filter(focal_sps == sps & training_set == set) %>%
  pull(str)
...
if(mod == "no_comp"){
  ...
  nci <- function(neighbors, alpha, beta, intra, inter){
    ...
    cons <- which(neighbors$sps_comp == sps)
    hets <- which(neighbors$sps_comp != sps)
...
train_r2 <- coef_det(train_obs_pred)
test_r2 <- coef_det(test_obs_pred)
